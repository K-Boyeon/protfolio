$(function(){
    ...
    $(window).resize(function(){ //이벤트 헨들링의 첫 시작이 setTimeout -> 이벤트 헨들링을 조종해주는 역할
		h = $(window).height();
		$("section").css({height : h});//css속성을 줌으로써 자동적으로 px단위를 인식함
	});
    ...
    tl.fromTo(innerH1, 0.7, {
        opacity: 1
    },{
        opacity: 0
    });
    tl.fromTo(mainMenu, 0.5, {
        opacity: 0
    },{
        opacity: 1,
        ease: Power2.easeIn
    ...
    $("html, body").mousewheel(function(e, delta){
        ...
        $("html, body").animate({scrollTop:$("#section" + (n + 1)).offset().top}, 800, function(){
    ...
    $(".menu li a").click(function(e){
        e.preventDefault();
        n = $(this).parent().index();
        $("html, body").animate({scrollTop: $("#section" + (n + 1)).offset().top}, 800);
        $(".fixed-menu > .menu li a").removeClass("on");
        $(".fixed-menu > .menu li a").eq(n).addClass("on");
    });
    ...
    $(function() {
        $('progress').each(function() {
            var max = $(this).val();
            $(this).val(0).animate({ value: max }, { duration: 8000, easing: 'easeOutCirc' });
        });
    });
